...
| **Skill Type** | Offensive / Utility |
| **Skill Check** | 0–125 (varies by poison type) |
| **Cooldown** | 5 seconds |
...
Poisoning is the skill used to coat weapons, food, and beverages with poison from potions. The resulting poisoned items deal additional damage when used in combat or consumed. Higher Poisoning skill allows application of stronger poisons and increases the effectiveness of poison-related weapon abilities.
...
- On self-poison (failure at skill < 80), the effect matches the poison level you were attempting to apply.
- Classic Poisoning mode is controlled by a player preference setting; see `Settings.cs` for configuration options.
- Poison level delivery on weapon abilities is capped at `Poisoning.Fixed / 200`, meaning you need at least 800 Poisoning to consistently deliver Lethal poison via abilities.
