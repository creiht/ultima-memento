...
Peacemaking pacifies hostile creatures either in an area (target yourself) or on a single target (target a creature). Against players with insufficient Magic Resist, it causes paralysis. A successful Musicianship instrument check must pass before the Peacemaking check is attempted.
...
- `[Discordance](discordance.md)`: Weakens targets' resistances instead of pacifying. Shares the same Musicianship gate.
...
- Peacemaking requires an **instrument** in the backpack to use.
- A successful **Musicianship instrument check** is a prerequisite for every Peacemaking attempt.
- Minimum Peacemaking skill required = `(instrument difficulty for target - 25)`.
- `[Begging](begging.md)`: Can also pacify creatures without an instrument (alternative method).
- `[Healing](healing.md)`: Listed alongside Peacemaking in the Behavior AI skill pool.

## Notes

- Area Peacemaking carries a 50% risk of stopping the song entirely if a creature's check fails.
- Single-target mode gives significantly longer pacification (up to 120 seconds) than area mode (`Musicianship / 10` seconds).
- Against players, Peacemaking causes paralysis rather than pacification, bypassing normal creature immunity flags.
