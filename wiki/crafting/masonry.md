# Masonry

Masonry (Stone Working) creates stone furniture, statues, containers, and tombstones. Requires Carpentry 100+ and the Masonry skill (learned from a book).

- **Skill**: Carpentry (minimum 100.0 base)
- **Tool**: Mallet and Chisel
- **Primary Resource**: Granite
- **Prerequisite**: Must learn Masonry from a special book
- **Supports**: Breakdown

## Materials

Masonry uses **Granite** — each granite tier corresponds to the same ore tier mined in [Mining](mining.md). Granite is produced when Mining at 100+ skill with the Stone Mining toggle enabled (10% chance per swing).

### Granite Tier Reference

| Granite | Corresponding Ore | Source Region |
|---|---|---|
| Standard (Iron) | Iron | Any mining tile |
| Dull Copper | Dull Copper | Any mining tile |
| Shadow Iron | Shadow Iron | Any mining tile |
| Copper | Copper | Any mining tile |
| Bronze | Bronze | Any mining tile |
| Gold | Gold | Any mining tile |
| Agapite | Agapite | Any mining tile |
| Verite | Verite | Any mining tile |
| Valorite | Valorite | Any mining tile |
| Nepturite | Nepturite | Sea areas / Shipwreck Grotto / Barnacled Cavern |
| Obsidian | Obsidian | Serpent Island |
| Steel | Steel | Savaged Empire |
| Brass | Brass | Umber Veil |
| Mithril | Mithril | Underworld (non-Savaged maps) |
| Xormite | Xormite | Underworld (Savaged Empire map) |
| Dwarven | Dwarven | Rich Vein nodes only |

Full ore/granite tier stats are on the [Mining](mining.md) page.

## Granite Types (legacy list)

Standard, Dull Copper, Shadow Iron, Copper, Bronze, Gold, Agapite, Verite, Valorite, Nepturite, Obsidian, Steel, Brass, Mithril, Xormite, Dwarven

## Containers

| Item | Min Skill | Granite | Notes |
|---|---|---|---|
| Urn | 80.0 | 5 | |
| Vase | 80.0 | 5 | |
| Ornate Urn | 90.0 | 6 | |
| Ornate Tall Vase | 95.0 | 8 | |
| Sarcophagus (woman) | 90.0 | 10 | Requires Forensics 75 |
| Sarcophagus (man) | 90.0 | 10 | Requires Forensics 75 |

## Decorations & Furniture

Vases, amphoras, sculptures (Buddhist, Ming, Yuan, Qin), urns, chairs, benches, tables, columns, pedestals, pillars, steps, and blocks (42.5-95.0 skill, 2-20 granite).

## Statues

- **Small Statues** (55.0 skill, 4 granite): Angel, Dragon, Gargoyle, Man, Noble, Pegasus, Skull, Woman
- **Medium Statues** (65.0 skill, 8-10 granite): ~20 varieties including Adventurer, Amazon, Fighter, Gryphon, Medusa, Sphinx, Wizard
- **Large Statues** (75.0 skill, 16 granite): Dwarf, God, Horse Rider, Lion, Minotaur, Pegasus, Woman Warrior
- **Huge Statues** (85.0 skill, 24 granite): Angel, Daemon, Lion, Warrior, Wise Man, Wolf, Woman
- **Giant Statues** (95.0 skill, 32 granite): Gate Guardian, Guardian, Warrior

## Tombstones

20 varieties, all at 45.0 skill using 3 granite each.
